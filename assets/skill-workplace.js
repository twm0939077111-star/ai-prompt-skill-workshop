(() => {
  ...
  const pages=[
    {id:'local_skill_definition',anchor:'local_skill_opening',title:'Skill 是一份可重複使用的工作方法',tag:'先理解概念',lead:'把做事步驟、範本與檢查標準，整理成 AI 能使用的資料包。',body:grid(card('指令｜怎麼做','先讀什麼、依序做什麼、哪些事不能自行猜測。'),card('資源｜照什麼做','放入範本、參考文件與好範例；需要時才加腳本。'),card('啟用｜何時使用','系統先看名稱與描述；選用後，再讀取完整指引。'),card('價值｜下次能重用','每次換新資料，沿用同一套流程；成果仍要檢查。')),note:'Skill 不等於重新訓練模型，也不保證每次輸出完全相同。',source:'build'},
    {id:'local_skill_roles',anchor:'local_skill_storyboard',title:'任務、方法、資料、工具，各有角色',tag:'不要混在一起',lead:'以「整理本週工作報告」為例，四個部分一起配合。',body:grid(card('Prompt＝這次要做什麼','「整理本週進度，給主管看，控制在一頁。」'),card('Skill＝固定怎麼做','依專案分類 → 找出風險 → 列出下週行動 → 檢查。'),card('資料＝這次根據什麼','本週紀錄提供事實；記憶／自訂指示提供長期偏好。'),card('工具＝能執行什麼','讀檔、查資料、建立文件；須有可用工具與相應權限。')),note:'Plugin（外掛）是安裝用的組合包，可以包含 Skill 與工具連接。',source:'use'},
    {id:'local_skill_jobs',anchor:'local_ppt_skill_intro',title:'哪些工作，適合做成 Skill？',tag:'從日常工作找題目',lead:'常常重做、標準清楚、成果能檢查，就是好的起點。',body:grid(card('會議追蹤','輸入會議紀錄 → 區分決議與提案 → 產出待辦表。'),card('每週工作報告','輸入進度紀錄 → 整理成果與風險 → 產出週報。'),card('行銷企劃','輸入產品與受眾 → 核對賣點與限制 → 產出企劃初稿。'),card('客服回覆草稿','輸入客訴與政策 → 核對可承諾事項 → 產出回覆草稿。')),note:'先選一項小工作。偶爾問一次的問題，通常直接寫 Prompt 就夠了。',source:'use'},
    {id:'local_skill_use',anchor:'local_skill_jobs',title:'實際使用 Skill，照這四步走',tag:'從選用到交付',lead:'先確認你的產品介面支援 Skills，而且已安裝需要的 Skill。',body:grid(card('01 選用方法','ChatGPT 用 @ 選取；Codex 用 $ 指定。也可能自動選用。'),card('02 提供本次資料','附上文件，說明目標、讀者、期限與希望的成果格式。'),card('03 讓 AI 依流程執行','缺少資料就補充；需要工具時，確認連接與權限可用。'),card('04 檢查後再交付','核對人名、數字、來源與承諾；不合適就要求修正。')),note:'只貼上 SKILL.md 文字，不等於已在任何 AI 產品中完成安裝。',source:'use'},
