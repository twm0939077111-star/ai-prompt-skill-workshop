(() => {
  ...
  },
  {
    "title": "應用二｜PPT 簡報製作",
    "ids": [
      "local_ppt_chapter_opening",
      "local_skill_theme02",
      "local_skill_theme08",
      "local_skill_theme09",
      "local_practice_marketing_deck",
      "local_apple_result_demo"
    ]
  }
];
  ...
  const openers = [{"id":"local_weekly_chapter_opening","title":"工作週報 Skill","tag":"應用 01｜工作週報"},{"id":"local_ppt_chapter_opening","title":"PPT Skill","tag":"應用 02｜簡報製作"}];
  for(const item of openers){
    ...
    node.innerHTML=`<div class="dso-accent-field" aria-hidden="true"></div><header class="dso-top"><span>SKILL APPLICATION</span><b>${weekly?'01':'02'}</b></header><main class="dso-copy"><p class="dso-intro">現在介紹</p><h1>${weekly?'工作週報':'PPT'}<br><strong>Skill<span class="dso-title-dot">.</span></strong></h1><p class="dso-description">${weekly?'把零散工作紀錄，<br>整理成清楚的一頁週報。':'把內容與素材，<br>變成有邏輯的視覺表達。'}</p></main>${art}<footer class="dso-bottom"><span>${weekly?'WORK → REPORT':'IDEAS → SLIDES'}</span><span>${weekly?'工作應用｜週報':'工作應用｜簡報'}</span></footer>`;
  ...
  const teachingNewIds = ["local_apple_result_demo"];
  for(const id of teachingNewIds){const node=document.createElement('section');node.className='slide';node.dataset.vmSlideId=id;node.dataset.vmSlideKey=id;deck.appendChild(node);model.slides.push({id,key:id,layout:'LOCAL-PPT-LESSON',dataLayout:'LOCAL-PPT-LESSON',themePack:'theme01',label:'實作成果：Apple 公司介紹',props:{},media:{}});}
  window.applyHtmlPptTeaching(deck,model);
  ...
  model.exportId='html-ppt-human-five-20260911-47pages';el.textContent=JSON.stringify(model);
})();
