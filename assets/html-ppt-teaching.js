...
window.applyHtmlPptTeaching = function(deck, model) {
  ...
  const shots={'apple-step1.jpg':'assets/html-ppt-tutorial/apple-step1.jpg','apple-step2.jpg':'assets/html-ppt-tutorial/apple-step2.jpg'};
  const results=['assets/html-ppt-tutorial/apple-result-1.png','assets/html-ppt-tutorial/apple-result-2.png','assets/html-ppt-tutorial/apple-result-3.png','assets/html-ppt-tutorial/apple-result-4.png'];
  const dialogue={
    'apple-step1.jpg':{
  ...
  resultPage.body='<div class="pl-result-grid">'+['Apple 概覽','產品與服務','營運模式','全球影響力'].map((title,i)=>`<figure><a href="${results[i]}" target="_blank" rel="noopener"><img src="${results[i]}" alt="第 ${i+1} 頁：${title}"></a><figcaption><b>0${i+1}</b> ${title}</figcaption></figure>`).join('')+'</div>';
  ...
  for(const page of pages){
    ...
    const shot=`<div class="pl-evidence"><h3 class="pl-column-title">實際操作畫面 <small>原始對話截圖</small></h3><div class="pl-shot-space"><a class="pl-crop pl-single-shot" href="${shots[file]}" target="_blank" rel="noopener" aria-label="放大查看完整原始截圖" style="aspect-ratio:${w}/${h}"><img src="${shots[file]}" alt="${first?'首次提問與 AI 建議':'補充條件與 AI 完成回覆'}，原始操作截圖" style="width:${2048/w*100}%;left:${-x/w*100}%;top:${-y/h*100}%"></a></div></div>`;
